...
func (m *BaseMaster) isJobManager() bool {
	return m.masterID == ""
}
...
func (m *BaseMaster) registerHandlerForMaster(ctx context.Context) error {
...
func (m *BaseMaster) runHeartbeatWorker(ctx context.Context) error {
...
func (m *BaseMaster) runWatchDog(ctx context.Context) error {
	ticker := m.clock.Ticker(m.timeoutConfig.workerHeartbeatInterval)
	for {
		select {
		case <-ctx.Done():
