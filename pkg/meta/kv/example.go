...
func Test(t *testing.T) {
	cli := mockclient.NewMetaMock()
	defer cli.Close()
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
	// nolint:typecheck
	var (
		putRsp *kvclient.PutResponse
		getRsp *kvclient.GetResponse
		delRsp *kvclient.DeleteResponse
	...
	//
	// Key Range, forbit Put using kvclient.WithRange
	// current data:
	//		apple  red
	//		orange orange
	//		ticdc  kv
	//		dm	   DDL
	getRsp, err = cli.Get(ctx, "ap", kvclient.WithRange("zz"))
	// expect len(getRsp.Kvs) == 4(apple. orange, ticdc, dm)
	getRsp, err = cli.Get(ctx, "ap", kvclient.WithRange("apple2"))
	// expect len(getRsp.Kvs) == 1(apple)
	delRsp, err = cli.Delete(ctx, "dzst", kvclient.WithRange("panda"))
	// delete key orange

	// From Key, forbit Put using kvclient.WithFromKey
	// current data:
	//		apple  red
	//		ticdc  kv
	//		dm	   DDL
	getRsp, err = cli.Get(ctx, "data", kvclient.WithFromKey())
	// expect len(getRsp.Kvs) == 2(ticdc, dm)
	delRsp, err = cli.Delete(ctx, "tian", kvclient.WithFromKey())
	// delete key ticdc

	// Key Prefix, forbit Put using kvclient.WithPrefix
	// current data:
	//		apple  red
	//		apple2  green
	//		ticdc  kv
	//		dm	   DDL
	getRsp, err = cli.Get(ctx, "apple", kvclient.WithPrefix())
	// expect len(getRsp.Kvs) == 2(apple, apple2)
	delRsp, err = cli.Delete(ctx, "apple", kvclient.WithPrefix())
	...
	//		ticdc  kv
	//		dm	   DDL
	getOp := kvclient.OpGet("apple3", kvclient.WithRange("zz"))
	_ = getRsp
	putOp := kvclient.OpPut("apple3", "t3")
	_ = putRsp
	delOp := kvclient.OpDelete("apple3", kvclient.WithRange("ti"))
	_ = delRsp
	_ = err
	txn := cli.Txn(ctx)
