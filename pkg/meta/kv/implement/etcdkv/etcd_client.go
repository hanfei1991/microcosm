...
func NewEtcdImpl(config *metacom.StoreConfigParams) (*etcdImpl, error) {
...
func (c *etcdImpl) getEtcdOptions(op kvclient.Op) []clientv3.OpOption {
...
func (c *etcdImpl) getEtcdOp(op kvclient.Op) clientv3.Op {
...
func (c *etcdImpl) Put(ctx context.Context, key, val string) (*kvclient.PutResponse, kvclient.Error) {
	op := kvclient.OpPut(key, val)
...
func (c *etcdImpl) Get(ctx context.Context, key string, opts ...kvclient.OpOption) (*kvclient.GetResponse, kvclient.Error) {
	op := kvclient.OpGet(key, opts...)
...
func (c *etcdImpl) Delete(ctx context.Context, key string, opts ...kvclient.OpOption) (*kvclient.DeleteResponse, kvclient.Error) {
	op := kvclient.OpDelete(key, opts...)
	if err := op.CheckValidOp(); err != nil {
		return nil, &etcdError{
			displayed: cerrors.ErrMetaOptionInvalid.Wrap(err),
		}
...
func (c *etcdImpl) Do(ctx context.Context, op kvclient.Op) (kvclient.OpResponse, kvclient.Error) {
	if err := op.CheckValidOp(); err != nil {
		return kvclient.OpResponse{}, &etcdError{
			displayed: cerrors.ErrMetaOptionInvalid.Wrap(err),
		}
	}
	...
	etcdResp, err := c.cli.Do(ctx, c.getEtcdOp(op))
	if err != nil {
		return kvclient.OpResponse{}, etcdErrorFromOpFail(err)
	}
...
func (c *etcdImpl) Txn(ctx context.Context) kvclient.Txn {
	return &etcdTxn{
		Txn: c.cli.Txn(ctx),
		kv:  c,
...
func (t *etcdTxn) Do(ops ...kvclient.Op) kvclient.Txn {
	t.mu.Lock()
	defer t.mu.Unlock()
...
func (t *etcdTxn) Commit() (*kvclient.TxnResponse, kvclient.Error) {
	t.mu.Lock()
	if t.Err != nil {
		t.mu.Unlock()
		return nil, t.Err
