...
func NewMetaOpsClient(dsn string) (*MetaOpsClient, error) {
	// TODO: configable db params
	sqlDB, err := newSQLDB("mysql", dsn, newDefaultDbConfig())
	if err != nil {
		return nil, err
	}

	return newMetaClient(sqlDB)
}
...
func newSQLDB(driver string, dsn string, conf *DBConfig) (*sql.DB, error) {
	...
	if err != nil {
		log.L().Error("connect backend metastore fail", zap.String("dsn", dsn),
			zap.Error(err))
...
func newMetaClient(sqlDB *sql.DB) (*MetaOpsClient, error) {
...
func (c *MetaOpsClient) Initialize(ctx context.Context) error {
	if err := c.db.AutoMigrate(&model.ProjectInfo{}, &model.ProjectOperation{}, &model.MasterMeta{},
		&model.WorkerStatus{}, &model.ResourceMeta{}); err != nil {
...
func (c *MetaOpsClient) AddWorker(ctx context.Context, worker *model.WorkerStatus) error {
	if worker == nil {
...
func (c *MetaOpsClient) UpdateWorker(ctx context.Context, worker *model.WorkerStatus) error {
	// TODO: shall we change the model name?
...
func (c *MetaOpsClient) DeleteWorker(ctx context.Context, masterID string, workerID string) error {
	if result := c.db.Where("job_id = ? AND id = ?", masterID,
		workerID).Delete(&model.WorkerStatus{}); result.Error != nil {
		return cerrors.ErrMetaOpFail.GenWithStackByArgs(result.Error)
	}
...
func (c *MetaOpsClient) GetWorkerByID(ctx context.Context, masterID string, workerID string) (*model.WorkerStatus, error) {
	var worker model.WorkerStatus
	if result := c.db.Where("job_id = ? AND id = ?", masterID,
...
func (c *MetaOpsClient) QueryWorkersByMasterID(ctx context.Context, masterID string) ([]model.WorkerStatus, error) {
...
func (c *MetaOpsClient) QueryWorkersByStatus(ctx context.Context, masterID string, status int) ([]model.WorkerStatus, error) {
	var workers []model.WorkerStatus
